...
command_line_helper::command_line_helper()
    ...
    main_options.add_options()
            ...
            ("cache-type", po::value<string>()->default_value("auto"),
             ...
             "'bitmap' (1-bit-per-entry transposition table, single-deck non-accordion games).")
...
bool command_line_helper::parse(int argc, const char* argv[]) {
    ...
    if (cache_type != "auto" && cache_type != "hash-only"
        ...
        LOG_ERROR ("Error: invalid --cache-type: " + cache_type + ". Must be 'auto', 'hash-only', 'multiplicity', or 'bitmap'.");
        return false;
    }
