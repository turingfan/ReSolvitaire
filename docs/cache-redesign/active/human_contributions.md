...
**Evidence**: Design discussion on 2026-04-04, building on hash-guarded second cache line (#25).

# 28. Hash-only cache streamliner with deferred verification

**Insight**: Refines #16 (1-bit payload) into a practical streamliner strategy. Store *only* the full 64-bit Zobrist hash as each cache entry — no payload at all. This gives 8 entries per 64-byte cache line (8× density over full-payload entries) and eliminates all payload computation. False positive rate is ~1/2^64 per probe (vastly better than the 1-bit approach of #16, whose false positive rate depends on bucket collision rate). Correctness: if a solution is found, it is definitely correct (the solver has the move sequence). If no solution is found, a false positive could in principle have pruned a winning path, but the expected count over a full search is negligible (~10^8/2^64 ≈ 5×10^-12). For batch solvability surveys, use a two-phase strategy: (1) run with hash-only cache (fast, 8× density), (2) for games classified unsolvable, re-run with full payload verification. Games classified solvable need no re-verification. The proportion of games requiring re-verification depends on the game — e.g., ~20% for Klondike, ~0.001% for FreeCell. Critically, the full-payload re-run is the cost that would have been paid without the streamliner; the actual overhead of the two-phase strategy is only the initial hash-only run, which is cheaper than baseline (no payload computation, higher cache density → fewer evictions → less re-search). The streamliner is therefore strictly better in expected cost than a single full-payload run.
